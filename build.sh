...
LCP_DIR=litecoinplus-testnet
LCPD_EXEC=litecoinplusd
...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/litecoinplus-qt.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    LCPD_EXEC=litecoinplusd.exe
fi
...
CONFIG="CONFIG+=";
$QMAKE ../litecoinplus.pro "$CONFIG" || exit
$MAKE || exit 

# Copy bittubed to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$LCP_DIR/bin/$LCPD_EXEC $BIN_PATH
fi
